...
INSERT INTO `creature_template`
  (`entry`, `difficulty_entry_1`, `difficulty_entry_2`, `difficulty_entry_3`,
   ...
   `ExperienceModifier`, `RacialLeader`, `movementId`, `RegenHealth`, `flags_extra`,
...
SELECT
  @NPC_ALLIANCE, 0, @SW_ZONE, @SW_AREA, `spawnMask`, @SW_PHASE, `equipment_id`,
  @SW_X, @SW_Y, @SW_Z, @SW_O,
...
FROM `creature`
WHERE `id` = @SW_GUARD
  AND NOT EXISTS (SELECT 1 FROM `creature` WHERE `id` = @NPC_ALLIANCE)
ORDER BY `guid`
LIMIT 1;
...
INSERT INTO `creature`
  (`id`, `map`, `zoneId`, `areaId`, `spawnMask`, `phaseMask`, `equipment_id`,
   ...
   `ScriptName`, `VerifiedBuild`, `CreateObject`, `Comment`)
SELECT
  @NPC_HORDE, 1, @ORG_ZONE, @ORG_AREA, `spawnMask`, @ORG_PHASE, `equipment_id`,
...
FROM `creature`
WHERE `id` = @ORG_GRUNT
  AND NOT EXISTS (SELECT 1 FROM `creature` WHERE `id` = @NPC_HORDE)
ORDER BY `guid`
LIMIT 1;
